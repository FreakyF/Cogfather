    ...
    : IConsumer<ManufactureComponentCommand>
{
    ...
    [LoggerMessage(Level = LogLevel.Information, Message = "[{Node}] Received order: {Amount}x {ComponentId} (Task: {CorrelationId})")]
    private static partial void LogOrderReceived(ILogger logger, string node, double amount, string componentId, Guid correlationId);

    [LoggerMessage(Level = LogLevel.Information, Message = "[{Node}] Production finished for {CorrelationId}. Reporting back.")]
